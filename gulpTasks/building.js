...
const applicationName = (options.wallet) ? 'Ballzcoin-Ballzbrowser-Wallet' : 'Mist';


gulp.task('clean-dist', (cb) => {
    return del([
        `./dist_${type}/**/*`,
        './ballzwallet-dapp-wallet'
    ], cb);
});
...
gulp.task('bundling-interface', (cb) => {
    ...
    if (type === 'wallet') {
        if (options.walletSource === 'local') {
            console.log('Use local wallet at ../ballzwallet-dapp-wallet/app');
            bundle(`&& cd ../../ballzwallet-dapp-wallet/app \
                && meteor-build-client ../../ballzbrowser/dist_${type}/app/interface/wallet -p ""`);
        } else {
            console.log(`Pulling https://github.com/ballzcoin/ballzwallet-dapp-wallet/tree/${options.walletSource} "${options.walletSource}" branch...`);
            bundle(`&& cd ../dist_${type} \
                && git clone --depth 1 https://github.com/ballzcoin/ballzwallet-dapp-wallet.git \
                && cd ballzwallet-dapp-wallet/app \
                && meteor-build-client ../../app/interface/wallet -p "" \
                && cd ../../ \
                && rm -rf ballzwallet-dapp-wallet`);
        }
    } else {
        bundle();
    }
...
gulp.task('build-dist', (cb) => {
    const appPackageJson = _.extend({}, require('../package.json'), {  // eslint-disable-line global-require
        ...
        homepage: 'https://github.com/ballzcoin/ballzbrowser',
        build: {
