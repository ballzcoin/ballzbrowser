...
gulp.task('update-nodes', (cb) => {
    const clientBinariesGeth = clientBinaries.clients.Ballzcoin;
    ...
    got('https://api.github.com/repos/ballzcoin/ballzcoin/releases/latest', { json: true })
    .then((response) => {
    ...
    .then((tagName) => {
        ...
        if (cmp(latestGethVersion, localGethVersion)) {
            ...
            got(`https://api.github.com/repos/ballzcoin/ballzcoin/commits/${tagName}`, { json: true })
            .then((response) => {
