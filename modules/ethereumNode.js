...
const DEFAULT_NODE_TYPE = 'ballzcoin';
...
class EthereumNode extends EventEmitter {
    ...
    _start(nodeType, network, syncMode) {
        ...
        return this.stop()
            ...
            .catch((err) => {
                ...
                if (nodeType === 'eth') {
                    Settings.saveUserData('node', 'ballzcoin');
                }
    ...
    __startProcess(nodeType, network, binPath, _syncMode) {
        ...
        if (nodeType === 'ballzcoin' && !syncMode) {
            syncMode = 'fast';
        }
            ...
            logRotate(Settings.constructUserDataPath('node.log'), { count: 5 }, (err) => {
                ...
                proc.stdout.on('data', (data) => {
                    ...
                    if (STATES.STARTING === this.state) {
                        ...
                        if (nodeType === 'ballzcoin') {
                            if (dataStr.indexOf('fatal: error') >= 0) {
                                const error = new Error(`Ballzcoin error: ${dataStr}`);
